                    ...
                    <div className="flex flex-wrap items-start gap-3">
                      <div className="min-w-0 grow">
                        ...
                        <p className="mt-1 flex items-center gap-1.5 text-[12px] text-[var(--ink-faint)]">
                          <UserRound className="h-3.5 w-3.5" /> Sent by {row.fromDisplayName || row.fromEmail || row.fromId.slice(0, 8)} · {new Date(row.createdAt).toLocaleString("en-US", { timeZone: "America/Detroit" })}
                        </p>
                      </div>
              ...
              <div className="grid gap-2">
                {sentQs.map((row) => (
                  <div key={row.id} className="flex items-center gap-2 rounded-[6px] border border-[var(--line-soft)] bg-[var(--paper-soft)] px-3 py-2 text-[12px] text-[var(--ink-faint)]">
                    ...
                    <span className="ml-auto">{new Date(row.createdAt).toLocaleDateString("en-US", { timeZone: "America/Detroit" })}</span>
                  ...
                  <div className="flex items-start gap-3">
                    <div className="min-w-0 grow">
                      ...
                      <div className="text-[12.5px] text-[var(--ink-faint)]">
                        {row.questionCount} question{row.questionCount === 1 ? "" : "s"} · Shared by {row.fromDisplayName || row.fromEmail || row.fromId.slice(0, 8)} · {new Date(row.createdAt).toLocaleString("en-US", { timeZone: "America/Detroit" })}
                      </div>
